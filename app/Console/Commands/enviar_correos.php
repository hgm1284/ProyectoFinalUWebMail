...
class enviar_correos extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($emails  as $mail ) {
          // $data = DB::table('users')->select('email') ->where('id', '=', $mail->id_owner)->get();
        //     $mailArray =array('id'=> $mail->id,'mensaje'=> $mail->mensaje,'subject'=> $mail->subject,
        // 'content'=> $mail->content,'from'=>$data[0]->email);
        ...
        Mail::send('emails.enviar', $mailArray, function ($message) use
        ($mailArray) {
